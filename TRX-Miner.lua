...
local run_amount = ...
local whitelist_drops = { "#minecraft:coal_ores", "#minecraft:copper_ores", "#minecraft:diamond_ores",
	"#minecraft:emerald_ores", "#minecraft:gold_ores", "#minecraft:iron_ores", "#minecraft:lapis_ores",
	"#minecraft:redstone_ores", "minecraft:ancient_debris", "minecraft:nether_quartz_ore" }
...
function clean_trash()
	for slot = 4, 16, 1 do
		...
		if turtle.getItemCount(slot) > 0 then
			...
			if replace_lava and is_in_whitelist(whitelist_area_filler, turtle.getItemDetail(slot, true)) and
				slot_count <= filler_slots and item_count <= (filler_slots * 64) then
				--Ich brauche mehr Schokolade...
			elseif replace_lava and is_in_whitelist(whitelist_area_filler, turtle.getItemDetail(slot, true)) and
...
function needFuel(amount)
	...
	if fuel >= amount then
		...
		if turtle.getItemCount(3) >= 1 and turtle.getFuelLevel() < amount and
			turtle.getItemDetail(3, true).tags["minecraft:coals"] == true then
			turtle.refuel(63)
		end
