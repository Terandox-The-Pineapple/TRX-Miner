...
local whitelist_compact = { "minecraft:diamond", "minecraft:lapis_lazuli", "minecraft:raw_iron", "minecraft:redstone",
	"minecraft:raw_gold", "minecraft:gold_nugget", "minecraft:emerald", "minecraft:raw_copper", "minecraft:coal",
	"minecraft:gold_ingot" }
local whitelist_keep = { "minecraft:quartz", "minecraft:diamond_block", "minecraft:lapis_block",
	"minecraft:raw_iron_block", "minecraft:redstone_block", "minecraft:raw_gold_block", "minecraft:gold_ingot",
	"minecraft:gold_block", "minecraft:emerald_block", "minecraft:raw_copper_block", "minecraft:coal_block", "+compactable",
	"+ores" }
local whitelist_only_vein_mining = { "minecraft:obsidian" }
local whitelist_area_filler = { "minecraft:deepslate", "minecraft:cobbled_deepslate", "minecraft:stone",
	"minecraft:cobblestone", "minecraft:netherrack", "#minecraft:nylium" }
...
if fs.exists("trx-miner.conf") == false then
	...
	if new_options["whitelists"] ~= nil then
		...
		if new_options["whitelists"]["only_vein_mining"] ~= nil then whitelist_only_vein_mining = new_options["whitelists"][
				"only_vein_mining"]
		end
		if new_options["whitelists"]["area_filler"] ~= nil then whitelist_area_filler = new_options["whitelists"][
				"area_filler"]
		end
		if new_options["whitelists"]["replace_lava"] ~= nil then whitelist_replace_lava = new_options["whitelists"][
				"replace_lava"]
		end
	end
	...
	if new_options["vein_mining"] ~= nil then
		if new_options["vein_mining"]["vein_check_limit"] ~= nil then max_ore_check = new_options["vein_mining"][
				"vein_check_limit"]
		end
		if new_options["vein_mining"]["enabled"] ~= nil then do_vein_mining = new_options["vein_mining"]["enabled"] end
		if new_options["vein_mining"]["do_only_vein_mining"] ~= nil then only_vein_mining = new_options["vein_mining"][
				"do_only_vein_mining"]
		end
	end
	if new_options["mining"] ~= nil then
		...
		if new_options["mining"]["funny_chunk_loading"] ~= nil then funny_chunk_loading = new_options["mining"][
				"funny_chunk_loading"]
		end
	end
	if new_options["replace_lava"] ~= nil then
		...
		if new_options["replace_lava"]["filler_slots"] ~= nil then filler_slots = tonumber(new_options["replace_lava"][
				"filler_slots"])
		end
	end
...
function storage_interact()
	...
	for slot = first_storage_slot, last_storage_slot, 1 do
		if chest.getItemDetail(slot) ~= nil then
			if is_in_whitelist(whitelist_compact, chest.getItemDetail(slot)) == false or chest.getItemDetail(slot).count < 9 then
				--The cake is a lie
			else
				...
				if pushed == true then
					for i = first_storage_slot, last_storage_slot, 1 do
						if chest.getItemDetail(i) ~= nil then
							if chest.getItemDetail(i).name == chest.getItemDetail(2).name and
								(chest.getItemDetail(i).count + chest.getItemDetail(2).count) <= 64 then
				...
				for i = first_storage_slot, last_storage_slot, 1 do
					if chest.getItemDetail(i) ~= nil then
						if chest.getItemDetail(i).name == chest.getItemDetail(1).name and
							(chest.getItemDetail(i).count + chest.getItemDetail(1).count) <= 64 then
...
function clean_trash()
	for slot = 4, 16, 1 do
		...
		if turtle.getItemCount(slot) > 0 then
			...
			if replace_lava and is_in_whitelist(whitelist_area_filler, turtle.getItemDetail(slot)) and
				slot_count <= filler_slots and item_count <= (filler_slots * 64) then
				--Ich brauche mehr Schokolade...
			elseif replace_lava and is_in_whitelist(whitelist_area_filler, turtle.getItemDetail(slot)) and
				item_count > (filler_slots * 64) then
				turtle.drop(item_count - (filler_slots * 64))
			elseif is_in_whitelist(whitelist_keep, turtle.getItemDetail(slot)) == false then
				turtle.drop()
			end
		end
	end
...
function scan_me_senpai()
	...
	while true do
		if scaned < max_ore_check then
			for i = 1, max_ore_check, 1 do
				...
				if way ~= false then
					...
					dig_direction(way)
					scan_log.write("Create: matrix[" .. nz .. "][" .. ns ..
						"][" .. nh .. "] -M " .. (max + 1) .. " -# " .. #matrix .. " -W " .. way .. "\n")
				else
					break
				end
			end
		...
		if m_max > 1 then
			...
			scan_log.write("Delete: matrix[" ..
				zeile .. "][" .. spalte .. "][" .. hoehe .. "] -M " .. m_max .. " -# " .. #matrix .. " -L " .. last .. "\n")
		elseif m_max == 1 then
...
function get_building_blocks()
	for slot = 4, 16, 1 do
		if turtle.getItemDetail(slot, true) ~= nil then
			if is_in_whitelist(whitelist_area_filler, turtle.getItemDetail(slot)) then return slot end
		end
	end
	return false
end
...
function count_matching_slots(whitelist)
	...
	for slot = 4, 16, 1 do
		if turtle.getItemDetail(slot) ~= nil then
			if is_in_whitelist(whitelist, turtle.getItemDetail(slot)) then
...
function do_dig()
	...
	if is_block then
		...
		if is_in_whitelist(whitelist_drops, data) then
			turtle.select(1)
			turtle.dig()
		elseif replace_lava == true and is_in_whitelist(whitelist_area_filler, data) and slot_count <= filler_slots and
			item_count < (filler_slots * 64) then
			turtle.select(1)
			turtle.dig()
		else
...
function do_dig_up()
	...
	if is_block_up then
		...
		if is_in_whitelist(whitelist_drops, data_up) then
			turtle.select(1)
			turtle.digUp()
		elseif replace_lava == true and is_in_whitelist(whitelist_area_filler, data_up) and slot_count <= filler_slots and
			item_count < (filler_slots * 64) then
			turtle.select(1)
			turtle.digUp()
		else
...
function do_dig_down()
	...
	if is_block_down then
		...
		if is_in_whitelist(whitelist_drops, data_down) then
			turtle.select(1)
			turtle.digDown()
		elseif replace_lava == true and is_in_whitelist(whitelist_area_filler, data_down) and slot_count <= filler_slots and
			item_count < (filler_slots * 64) then
			turtle.select(1)
			turtle.digDown()
		else
...
function needFuel(amount)
	...
	if fuel >= amount then
		...
		if turtle.getItemCount(3) >= 1 and turtle.getFuelLevel() < amount and
			turtle.getItemDetail(3).tags["minecraft:coals"] == true then
			turtle.refuel(63)
		end
...
function hasChest()
	...
	if turtle.getItemDetail(2).name == "minecraft:chest" or
		turtle.getItemDetail(2).name == "minecraft:barrel" and turtle.getItemCount(2) >= 2 then
		return true
	else
